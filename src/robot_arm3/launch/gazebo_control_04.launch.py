...
def generate_launch_description():
    ...
    xacro_file = os.path.join(package_share_dir, "urdf", "robot_arm3_control_04.urdf.xacro")
    ...
    spawn_robot = Node(
        package="ros_gz_sim",
        executable="create",
        name="spawn_robot_arm3_control_04",
        output="screen",
        arguments=[
            "-world",
            "empty",
    ...
    gazebo_initial_hold_publisher = Node(
        package="robot_arm3",
        executable="gazebo_initial_hold_publisher_03.py",
        name="gazebo_initial_hold_publisher_03",
        output="screen",
    )

    gazebo_passive_joint_controller = Node(
        package="robot_arm3",
        executable="gazebo_passive_joint_controller_03.py",
        name="gazebo_passive_joint_controller_03",
        output="screen",
    )
    ...
    start_passive_controller_after_initial_hold = RegisterEventHandler(
        OnProcessExit(
    ...
    return LaunchDescription(
        [
            ...
            start_passive_controller_after_initial_hold,
        ]
    )
