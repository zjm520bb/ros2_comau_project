...
def generate_launch_description():
    ...
    moveit_control_share = get_package_share_directory(
        "robot_arm3_moveit_control"
    )
    ...
    program_controller = Node(
        package="robot_arm3_moveit_control",
        executable="moveit_command_server",
        name="moveit_program_command_server",
        output="screen",
        parameters=[
            moveit_config.to_dict(),
            os.path.join(
                moveit_control_share,
                "config",
                "program_controller.yaml",
            ),
            {"use_sim_time": True},
        ],
    )
